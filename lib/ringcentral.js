...
// Reintenta una vez si choca con el limite de solicitudes de RingCentral (429).
async function getOutboundCallLog({ extensionId, phoneNumber, dateFrom }, retrying = false) {
  const token = await getAccessToken();
  ...
  if (res.status === 429 && !retrying) {
    // Limite de solicitudes -- esperar 1.5s y reintentar una sola vez.
    await new Promise((r) => setTimeout(r, 1500));
    return getOutboundCallLog({ extensionId, phoneNumber, dateFrom }, true);
  }
